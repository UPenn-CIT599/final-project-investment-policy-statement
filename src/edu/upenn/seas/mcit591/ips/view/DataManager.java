...
//	DataManager.updateTimeValueVars(Integer.parseInt(age), Double.parseDouble(currentAssets), Double.parseDouble(retiermentGoal), annuity, numberofYears, rateofReturnVar);
	...
	public static void updateTimeValueVars(int Userage, double currentAssetHoldingVar, double futureValueVar, double annuityVar,
		int numberofYearsVar, double rateofReturnVar) {
		age = Userage;
	...
	//get the Equality of Willingness and Ability to take risk for display in to View
	public static boolean getWillingAndAbilityEquality() {
		return willingAndAbilityEqual;
	}
	//set the Equality of Willingness and Ability to take risk for display in to View
	...
	//setter method to set the string that will be displayed based on IRR value.
	public static void setString() {
		
		
		if (IRR <= 0.19 && IRR > 0) {
			IRRstring = "<html>Based on the information you provided, your required rate of return is" + DataManager.getIRR() * 100 + "%.</html>";
	...
	private static String[] vehicleDescription = {"US Stock", "US Mid Cap", "US Small Cap", 
												"Treasury 1-3 years", "Total Bond", "20 year long-term bond", 
												"TIPS", "Municipal Bonds", "Foregin Market", "Foregin Market Small Cap", 
												"Emerging Market", "Real Estate", "Gold Community"};
	
	//Setting the fund names
	private static String[] fundName = {"Vanguard Total Stock Market ETF", "Vanguard Mid-Cap ETF", "Vanguard Small-Cap Index Fund", 
										"Barclays 1-3 Year Treasury Bnd", "Vangaurd Total Bond Market ETF", "iShares 20+ Year Treasury Bond ETF", 
										"iShares TIPS Bond ETF", "iShares National Municipal Bond ETF", "Vangaurd FTSE All-World ETF (no US)", "Vangaurd FTSE All-World Small Cap ETF (no US)",
	...
	//getters and setters to get data from above and set the data in RecommendPortfolioPanel class
...
//		Object[][] table = new Object[13][3];
		...
		for(int i=0; i<13; i++) {
			for(int j=0; j<3; j++) {
				switch(j) {
				case 0: table[i][j] = vehicleDescription[i];
				break;
				case 1: table[i][j] = FundPercent[i]*100;
				break;
				case 2: table[i][j] = fundName[i];
				break;
				}
